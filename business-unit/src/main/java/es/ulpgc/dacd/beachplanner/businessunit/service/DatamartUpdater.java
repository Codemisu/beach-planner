...
public class DatamartUpdater {
    ...
    public void update(Event event, Datamart datamart) {
        if (event.ss().equals("weather-module")) {
            updateWeather(event, datamart);
        }

        if (event.ss().equals("beachinfo-module")) {
            updateBeachInfo(event, datamart);
        }
    }

    private void updateWeather(Event event, Datamart datamart) {
        JsonObject payload =
                gson.toJsonTree(event.payload()).getAsJsonObject();
